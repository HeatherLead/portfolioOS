...
interface GamesProps {
  ...
  handleGamesClick: (value: number) => void;
  zIndex: number;
...
const Games = ({
  ...
  useBaseAnimations({ id: "games", screenWidth, t1Ref });

  const closeWindow = () => {
    if (t1Ref.current) {
      t1Ref.current.reverse().then(() => handleIconClick(State.Games));
    }
  };

  useDragger("gamesBar", "games");

  const { expanded, containerRef, newComponentRef, handleExpand } =
    expandedProps;
